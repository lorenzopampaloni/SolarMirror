...
def get_sun_position():
    LAT = 43.766  # Usa i tuoi valori reali
    LON = 11.222  # Usa i tuoi valori reali
    ALT = 45      # Usa i tuoi valori reali
    location = LocationInfo(latitude=LAT, longitude=LON)
    tz = pytz.timezone('Europe/Rome')  # Oppure usa il tuo fuso orario
    now = datetime.now(tz)
    observer = location.observer  # Coordinate dell'osservatore
...
def il_mio_codice():
    if __name__ == "__main__":
        main()
    print("Il mio codice sta funzionando!")

# --- Inizio dello scheduler semplice ---
